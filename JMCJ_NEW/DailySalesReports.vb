Public Class DailySalesReports
    ...
    Private Function generatePrint(ByVal filter As Dictionary(Of String, String))
        ...
        Dim query As String = "Select Format(cop.created_at,'mm-dd-yyyy') as daily,count(co.id) as orders, sum(cop.quantity) as qty, sum(cop.total_amount) as total_sales from customer_order_products as cop
                    inner join customer_orders as co ON co.id = cop.customer_order_id 
        ...
        With dbprod
            ...
            If .dr.HasRows Then
                While .dr.Read
                    Dim tr As String = "<tr>"
                    Dim daily As String = If(IsDBNull(.dr("daily")), "", .dr("daily"))
